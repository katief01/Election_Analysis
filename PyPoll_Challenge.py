...
with open(file_to_save, "w") as txt_file:
    ...
    for county_list in county_votes:
        ...
        county_votes[county_list] = (f"{county_list}: {vote_percentage:.1f}% ({votes:,})\n")
        print(county_votes[county_list])
        
         # 6e: Save the county votes to a text file.
        txt_file.write(county_votes[county_list])
        
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (votes > largest_turnout_votes):
            largest_turnout_votes = votes
            largest_turnout_county = county_list
